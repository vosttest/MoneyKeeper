...
BEGIN
RETURN QUERY
	SELECT a.id, a.type, a.text, a.balance + COALESCE(b.total, 0) as balance, a.currency, f.rate FROM account a
	LEFT JOIN	(
	...
	LEFT JOIN	(
					SELECT p.value, (p.text::FLOAT8/(
								...
								FROM common ch
								JOIN setting s ON s.text = 'Currency' AND ch.value = COALESCE(s.value, 'VND') AND s.user_id = uid
								WHERE ch.type = 'Rate')::FLOAT8) AS rate
					FROM common p
					WHERE p.type = 'Rate'
				) f ON f.value = a.currency
	WHERE a.user_id = uid
