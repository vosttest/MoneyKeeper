...
public interface IncomeDao extends CrudRepository<Income, Integer> {
	...
	@Query("FROM Income a WHERE a.userId = :id AND a.isDeleted = FALSE AND a.parentId is null order by a.id ASC")
	public List<Income> getParent(@Param("id") int id);

	@Query("FROM Income a WHERE a.userId = :id AND a.isDeleted = FALSE AND a.parentId is not null order by a.parentId ASC")
