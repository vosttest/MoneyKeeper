...
public interface ExpenseDao extends CrudRepository<Expense, Integer> {
	...
	@Query(nativeQuery = true, value = "SELECT a.id, code, text, parent_id, a.description, (SELECT COUNT(*) FROM income b WHERE a.id = b.parent_id AND b.parent_id IS NOT NULL AND b.is_deleted = FALSE), icon FROM Expense a WHERE a.user_id = :id AND a.is_deleted = FALSE AND a.parent_id IS NULL ORDER BY a.id ASC")
	public List<Object[]> getParent(@Param("id") int id);

	@Query(nativeQuery = true, value = "SELECT a.id, code, text, parent_id, a.description, (SELECT COUNT(*) FROM income b WHERE a.id = b.parent_id AND b.parent_id IS NOT NULL AND b.is_deleted = FALSE), icon FROM Expense a WHERE a.user_id = :id AND a.is_deleted = FALSE AND a.parent_id IS NOT NULL ORDER BY a.id ASC")
